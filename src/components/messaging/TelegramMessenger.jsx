...
export default function TelegramMessenger({ virtualNumberId, onMessageSent }) {
  ...
  const handleSend = async (e) => {
    ...
    try {
      const res = await base44.functions.invoke("sendTelegram", {
      ...
      if (res.data?.success) {
        setStatus({ type: "success", message: "Telegram sent!" });
        setChatId("");
        setMessage("");
