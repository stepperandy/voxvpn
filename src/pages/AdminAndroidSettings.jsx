...
import { Plus, Edit2, Trash2, Save, X, Loader2, Settings, RefreshCw } from "lucide-react";

export default function AdminAndroidSettings() {
  ...
  const loadSettings = async () => {
    try {
      setLoading(true);
      const data = await base44.asServiceRole.entities.AndroidSettings.list();
      setSettings(data || []);
    } catch (err) {
      console.error("Failed to load Android settings:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await base44.asServiceRole.entities.AndroidSettings.update(editingId, formData);
      } else {
        await base44.asServiceRole.entities.AndroidSettings.create(formData);
      }
      await loadSettings();
      resetForm();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Delete this setting?")) {
      try {
        await base44.asServiceRole.entities.AndroidSettings.delete(id);
        await loadSettings();
      } catch (err) {
  ...
  const handleAutoMigrate = async () => {
    if (!window.confirm("This will populate default Android settings if none exist. Continue?")) return;
    setMigrating(true);
    try {
      const res = await base44.functions.invoke('migrateAppSettings', {});
      const { results } = res.data;
      if (results.android.skipped) {
        alert(`Skipped: ${results.android.count} settings already exist.`);
      } else {
        alert(`Migrated ${results.android.migrated} default Android settings.`);
      }
      await loadSettings();
    } catch (err) {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-bold">Android Settings</h1>
          </div>
          <div className="flex gap-2">
            <button
              onClick={handleAutoMigrate}
              disabled={migrating}
              className="flex items-center gap-2 px-4 py-2.5 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white font-bold rounded-lg transition"
            >
        ...
        {showForm && (
          <div className="bg-white/5 border border-white/10 rounded-xl p-6 mb-8">
            <h2 className="text-xl font-bold mb-4">{editingId ? "Edit Setting" : "New Android Setting"}</h2>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., android:requestLegacyExternalStorage"
        ...
        {settings.length === 0 ? (
          <div className="text-center py-16 bg-white/5 border border-white/10 rounded-xl">
            ...
            <p className="text-gray-400 mb-4">No Android settings configured yet</p>
            <button
              onClick={() => setShowForm(true)}
